...
class PyDuiEmbeddedTextViewGTK3(PyDuiEmbeddedWidgetHostGTK3[PyDuiTextViewProtocol], PyDuiTextViewProtocol):
    ...
    def set_font(self, font: str, fontsize: int):
        ...
        self.__gtk_text_view.override_font(Pango.font_description_from_string(desc))  # type: ignore

    def set_editable(self, editable: bool):
        self.__gtk_text_view.set_editable(editable)

    def get_editable(self) -> bool:
        return self.__gtk_text_view.get_editable()  # type: ignore

    def set_size_request(self, width: float, height: float):
        self.__gtk_scrolled_window.set_size_request(width, height)  # type: ignore

    def __on_changed__(self, text_buffer: Gtk.TextBuffer):
        start: int = text_buffer.get_iter_at_offset(0)  # type: ignore
        end: int = text_buffer.get_iter_at_offset(-1)  # type: ignore
